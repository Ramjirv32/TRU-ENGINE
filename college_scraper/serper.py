...
COLLEGES = [
    {"name": "Udayana University", "country": "Indonesia", "location": "Denpasar"},
...
QUERIES = {
    ...
    "programs": "For the college named '%COLLEGE_NAME%', located in %COUNTRY% (%LOCATION%), provide the main 75 undergraduate, postgraduate, and PhD programs as of the 2026 academic year. If the college has more than 75 programs total, list the most important/most popular 75 and include a note with the total count. Use only official college website, latest accreditation reports, or recognized education sources. Return only valid JSON with keys: ug_programs (list of main undergraduate programs), pg_programs (list of main postgraduate programs), phd_programs (list of main doctoral programs), departments (list of main academic departments), total_programs_count (total number of programs offered by the college), and sources_verified (URLs or document names). If a level has no programs, use an empty array []. Do not invent programs - only list actual programs from official sources. Focus on the most significant programs offered.",
